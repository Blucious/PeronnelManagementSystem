...
create table administrator (
	admNo char(10)  primary key     			comment '管理号，必须大于0个字符且不能为空',
	admName nvarchar(32)  not null  			comment '管理员名，必须大于0个字符且不能为空',
	admHashedPassword varchar(128) not null     comment '管理员密码的数字摘要，不能为空',
	check(length(admNo) > 0), 
	check(length(admName) > 0)
) charset=utf8;
...
create table department (
    deptNo char(10) primary key     comment '部门号，必须大于0个字符且不能为空',
    deptName nvarchar(32) not null  comment '部门名字，必须大于0个字符且不能为空',
	check(length(deptNo) > 0),
	check(length(deptName) > 0) 
) charset=utf8;
